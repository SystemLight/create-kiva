...
/*
    安装模板项目
 */
function install(templateName = "react-kiva") {
    return new Promise((resolve, reject) => {
        const command = process.platform === "win32" ? "npm.cmd" : "npm";
...
/*
    复制模板项目到工作区
 */
function copyTemplate(templateName = "react-kiva") {
    const nowPath = process.cwd();
    const templatePath = ph.join(nowPath, "node_modules", templateName, "template");
    return new Promise((resolve, reject) => {
